...
pytestmark = pytest.mark.skipif(
    not LANGGRAPH_AVAILABLE, reason="LangGraph not installed (requires Python 3.9+)"
)
...
class TestLangGraphOrchestrator:
    ...
    def test_simple_workflow_execution(self):
        ...
        state = create_initial_state(intel_text="APT29 using PowerShell for initial access")
...
class TestErrorHandling:
    ...
    def test_missing_langgraph_import(self):
        ...
        with patch(
            "threat_research_mcp.orchestrator.langgraph_orchestrator.LANGGRAPH_AVAILABLE", False
        ):
