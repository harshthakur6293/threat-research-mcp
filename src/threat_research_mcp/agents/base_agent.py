...
class BaseAgent(ABC):
    ...
    def _validate_input(self, state: ThreatAnalysisState, required_fields: list) -> None:
        ...
        if missing_fields:
            raise ValueError(f"{self.name} missing required fields: {', '.join(missing_fields)}")

    def _create_output(
        self, findings: Dict[str, Any], confidence: float, metadata: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
