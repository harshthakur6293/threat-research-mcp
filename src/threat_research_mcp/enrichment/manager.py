...
class EnrichmentManager:
    ...
    def get_available_sources(
        self, ioc_type: Optional[IOCType] = None, tier: Optional[int] = None
    ...
    def enrich_ioc(
        self, ioc: str, ioc_type: IOCType, tiers: Optional[List[int]] = None
            ...
            for source in sources:
                try:
                    ...
                    if result.is_success():
                        ...
                        logger.warning(f"Failed to enrich {ioc} with {source.name}: {result.error}")

                except Exception as e:
                    logger.error(f"Error enriching {ioc} with {source.name}: {e}")
                    # Create error result
                    results.append(source._create_result(ioc=ioc, ioc_type=ioc_type, error=str(e)))

        return results

    def enrich_iocs(
        self, iocs: Dict[IOCType, List[str]], tiers: Optional[List[int]] = None
