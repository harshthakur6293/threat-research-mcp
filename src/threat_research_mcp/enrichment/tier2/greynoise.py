...
class GreyNoiseSource(EnrichmentSource):
    ...
    def enrich(self, ioc: str, ioc_type: IOCType) -> EnrichmentResult:
        if not self.can_enrich(ioc_type):
            return self._create_result(ioc=ioc, ioc_type=ioc_type, error="GreyNoise only supports IPs")
        ...
        return self._create_result(ioc=ioc, ioc_type=ioc_type, data=data, confidence=0.85, metadata={"mock": True})
