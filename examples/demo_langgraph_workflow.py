...
def demo_agent_communication():
    ...
    class HuntingAgentV2(BaseAgent):
        def execute(self, state: ThreatAnalysisState) -> ThreatAnalysisState:
            ...
            print(f"   🎯 Hunting Agent: Using {len(iocs)} IOCs and {len(techniques)} techniques from Research")
            
            state["hunt_plan"] = self._create_output(
                findings={
                    "hypotheses": [
                        f"Hunt for {technique} activity" for technique in techniques
                    ],
                    "based_on_iocs": iocs,
                    "queries": [
                        f"index=windows EventCode=4688 | search {ioc}" for ioc in iocs
                    ]
                },
                confidence=0.80
            )
